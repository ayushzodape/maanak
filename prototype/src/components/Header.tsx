...
import { Home, ShieldCheck, Scale, Database, BarChart3, UserCheck, History, ShieldAlert, LogOut } from 'lucide-react';
import { UserRole } from '../types';
import { AuthenticatedUser } from '../services/scanApi';

interface HeaderProps {
  currentTab: 'home' | 'history' | 'workbench' | 'repository' | 'dashboard' | 'admin';
  onSelectTab: (tab: 'home' | 'history' | 'workbench' | 'repository' | 'dashboard' | 'admin') => void;
  activeRole: UserRole;
            ...
            <div className="flex items-center gap-2 pl-2 border-l border-slate-800">
              <div className="text-right">
                ...
                <span className={`text-[9px] font-mono uppercase px-1 py-0.5 rounded ${
                  authenticatedUser.role === 'SUPERVISOR_ADMIN'
                    ? 'bg-amber-500/20 text-amber-300 border border-amber-500/40'
                    : 'bg-blue-500/20 text-blue-300 border border-blue-500/40'
                }`}>
                  {authenticatedUser.role === 'SUPERVISOR_ADMIN' ? 'Supervisor' : 'Inspector'}
                </span>
              </div>
              {onLogout && (
                <button
                  type="button"
                  onClick={onLogout}
                  title="Sign out of screening session"
                  className="flex items-center gap-1 text-[11px] text-slate-400 hover:text-rose-300 px-2 py-1 rounded bg-slate-800 border border-slate-700 hover:border-rose-700 transition-colors"
                >
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${
                  currentTab === 'admin'
                    ...
                    : 'text-amber-300 hover:text-white hover:bg-slate-800 border border-amber-500/40'
                }`}
              >
